...
function iniciarMicrófono() {
    ...
    navigator.mediaDevices
        ...
        .then(stream => {
            ...
            initVisualizacion(ANALIZADOR, DATOS);
        });
}
...
function procesarArchivo(file) {
    ...
    reader.onload = function (event) {
        ACTX.decodeAudioData(event.target.result, buffer => {
            const ANALIZADOR = ACTX.createAnalyser();
            ANALIZADOR.fftSize = 4 * frecuenciaMuestra;
            ANALIZADOR.smoothingTimeConstant = 0.5;
            const DATOS = new Uint8Array(ANALIZADOR.frequencyBinCount);

            const FUENTE = ACTX.createBufferSource();
...
function initVisualizacion(ANALIZADOR, DATOS) {
    let lut;
    ...
    function init() {
        ...
        for (let n = 0; n < 256; n++) {
            lut.push(
                new THREE.Vector3(
                    (cadena[n][0] * 255 - 49) / 206,
                    (cadena[n][1] * 255 - 19) / 236,
                    (cadena[n][2] * 255 - 50) / 190
                )
            );
        }
        ...
        function render() {
            actualizar_geometria(ANALIZADOR, DATOS);
            renderizador.render(escena, camara);
        }
    ...
    function actualizar_geometria(ANALIZADOR, DATOS) {
...
function anadirEtiquetas() {
    ...
    fontLoader.load('https://threejs.org/examples/fonts/helvetiker_regular.typeface.json', (font) => {
        const frecuenciasClave = ['20Hz', '160Hz', '640Hz', '2560Hz', '10240Hz'];
        frecuenciasClave.forEach((freq, i) => {
            const geometria = new THREE.TextGeometry(freq.toString(), {
                font: font,
                size: 0.8,
                height: 0.1,
            });
            ...
            mesh.position.set(xTamano2 / 2 + 1, -yTamano2 / 2 + (yTamano2 / frecuenciasClave.length) * (i + 0.5), 0);
            frecuenciaEtiquetas.push(mesh);
            escena.add(mesh);
        });
    });
}
...
mutedMicro.addEventListener("click", () => {
    if (fuente && volumenGanancia) {
        ...
        actMicrofono.style.display = "inline-block";
    }
});
